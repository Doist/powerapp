...
class TodoistAPI(todoist.TodoistAPI):
    ...
    def sync(self, commands=None, **kwargs):
        ...
        new_state = super(TodoistAPI, self).sync(commands, **kwargs)
    ...
    def emit_sync_signals(self, api_state, result):
        ...
        for result_key, event_type in result_event_map:
            for obj in result.get(result_key) or []:
                ...
                event_obj = self.integration.app_config.signals[event_name]

                logger.debug('Fire %s(%r, %r)' % (event_name,
                                                  self.integration.user.email,
                                                  obj['id']))
                resp = event_obj.send_robust(None,
